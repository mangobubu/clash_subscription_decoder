...
本系统支持**单文件一键全栈部署**。在编译后端时，前端静态资源会被静态嵌入到 Go 的可执行二进制文件中，您在生产环境部署时仅需要运行生成的单个二进制文件即可。

#### 步骤一：构建前端
在前端目录下运行构建命令，打包的静态资源会自动直出到后端嵌入目录 `backend/dist` 中：
```bash
cd frontend
pnpm build
```
...
#### 步骤二：编译并运行后端
在后端目录下编译项目，这会将刚才生成的前端资源连同 Go 代码一同编译进可执行文件：
```bash
cd backend
# 编译生成合并全量前端的可执行二进制文件 (Windows 环境下会自动生成 clash-proxy.exe)
