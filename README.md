...
本系统支持**单文件一键全栈部署**。为了简化部署流程，项目内置了一个纯 Go 编写的跨平台自动化构建工具。在编译后端时，前端静态资源会被静态嵌入到 Go 的可执行二进制文件中，您在生产环境部署时仅需要运行生成的单个二进制文件即可。

#### 步骤一：一键全栈构建
您无需分别安装前端和后端，也无需手动拷贝任何文件。只需在项目**根目录**下运行以下一行命令，构建系统将自动检测本地包管理器（优先使用 `pnpm`，若无则自动降级回退至 `npm`），自动拉取前端依赖、进行前端打包，并最终编译好 Go 后端：
```bash
go run build.go
```

#### 步骤二：运行全栈服务
构建成功后，在 `backend` 目录下会生成最终的单二进制可执行文件：
```bash
cd backend
# 运行全栈服务 (Windows 环境下文件名为 clash-proxy.exe)
./clash-proxy
```
运行后，访问 `http://localhost:8080` 即可直接使用完整的前端管理界面与后端 API 服务，完全免除 Nginx 等 Web 服务器分别部署的繁琐流程！
